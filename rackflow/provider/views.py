...
from .forms import ProductArrivalForm
...
class ShipmentUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('provider:list_shipments')  # Adjust to your order list URL name
    ...
    def form_valid(self, form):
        ...
        if self.object.status != "pending":
            form.add_error(None, f"Shipment with status {self.object.status} cannot be updated.")
            return self.form_invalid(form)
        valid_forms = 0
        for sub_form in shipment_product_formset:
            if sub_form.is_valid() and sub_form.cleaned_data and not sub_form.cleaned_data.get('DELETE'):
...
def update_shipment_status(request, id):
    ...
    Notification.objects.create(
        ...
        shipment=shipment,
        type = "shipment_accepted" if new_status == "accepted" else ("shipment_arrived" if new_status == "arrived" else "shipment_rejected")
    )
